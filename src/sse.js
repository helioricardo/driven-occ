...
const Methods = {
  createFolder: () => {
    return fs.mkdirSync(constants.dcu.paths.sse);
  },

  hasFolder: () => {
    return fs.existsSync(constants.dcu.paths.sse);
  },
  ...
  selector: async type => {
    ...
    if (!choices.length) {
      console.log(`Can't found server side extensions. Please check your ${constants.dcu.paths.sse} folder or your ${process.env.ACTIVE_ENV} environment.`);
      return false;
    }
  ...
  listLocal: () => {
    return fs.readdirSync(`${constants.dcu.paths.sse}`, {
        withFileTypes: true
      })
  ...
  get: async (sse, isBackup, env) => {
    ...
    if (sse) {
      ...
      if (response && response.data) {
        ...
        const writer = fs.createWriteStream(isBackup ? `${constants.dcu.paths.sse}/BKP-${env}-${timestamp}_${sse}` : `${constants.dcu.paths.sse}/${sse}`);
  ...
  delete: sse => {
    if (sse.indexOf('.zip') !== -1) {
      if (sse) {
        fs.unlinkSync(`${constants.dcu.paths.sse}/${sse}`);
      } else {
  ...
  zip: async sse => {
    ...
    const stream = fs.createWriteStream(`${constants.dcu.paths.sse}/${sse}.zip`);

    await new Promise((resolve, reject) => {
      archive
        .directory(`${constants.dcu.paths.sse}/${sse}`, false)
  ...
  unzip: async sse => {
    if (sse) {
      await extract(`${constants.dcu.paths.sse}/${sse}`, {
        dir: `${process.cwd()}/${constants.dcu.paths.sse}/${sse.replace('.zip', '')}`
      });
    } else {
  ...
  upload: async (sse, env) => {
    ...
    if (sse) {
      ...
      data.append('fileUpload', fs.createReadStream(`${constants.dcu.paths.sse}/${sse}`));
